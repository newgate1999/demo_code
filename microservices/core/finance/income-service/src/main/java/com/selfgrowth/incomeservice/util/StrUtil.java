package com.selfgrowth.incomeservice.util;
...
public class StrUtil {
    ...
    public static String createUrl(String title, Long id, String alias, String areaCode) {
        areaCode = areaCode != null ? areaCode.substring(areaCode.indexOf(".")) : "vn";
        String url = removeAccent(areaCode + "-" + title)
                .replaceAll(" ", "-")
                .replaceAll("[^a-zA-Z0-9-]", "")
                .toLowerCase();
        while (url.contains("--")) url = url.replaceAll("--", "-");
        if (url.startsWith("-")) url = url.replaceFirst("-", "");
        if (url.endsWith("-")) url = url.substring(0, url.length() - 1);
        if (url.isEmpty()) url = "view";
        return "/" + url + "-" + alias + id;
    }
}
